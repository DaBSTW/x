...
- [x] `PATCH /users/me` — display name, bio, ubicación, web, y ahora `avatarMediaId`/`bannerMediaId`: se resuelven contra la infraestructura de media de 1.5 (owned + `status = ready` + `kind = 'image'`, mismo criterio que adjuntar media a un post) y el servidor escribe la URL resuelta en `avatarUrl`/`bannerUrl` — el cliente nunca guarda una URL a mano. ⚪ Sin UI todavía: no existe una página de ajustes/edición de perfil en `apps/web` (1.8 nunca listó una entre sus bullets), así que esto es real y probado end-to-end a nivel de API pero no alcanzable como flujo de usuario. `<ProfileHeader>` sí lee y muestra `avatarUrl`/`bannerUrl` en cuanto existen.
