...
- [x] Badge "N posts nuevos" en el timeline vía canal `timeline:{id}` — primer publicador real de un evento de dominio sobre el gateway: `apps/workers`' `fanout.processor.ts` añade un `PUBLISH` (en el mismo pipeline que ya hacía `ZADD`/`ZREMRANGEBYRANK`/`EXPIRE`, sin round-trips extra) de `post.available` a `timeline:{followerId}` por cada seguidor, con un `eventId` (Snowflake fresco) ya listo para cuando exista recuperación por Streams. Best-effort a propósito: `PUBLISH` a un canal sin suscriptores no es un error, y el timeline ya quedó correcto por el `ZADD` sin importar si el evento llega. `<Timeline>` usa `useTimelineNewPosts` (nuevo) para contar eventos desde el último `reset()` y muestra un botón sticky arriba del feed; al pulsarlo hace `refetch()` de `['timeline','home']` y sube el scroll. ⚪ Cuentas célebres (fan-out on read) no publican nada todavía — quedan fuera del mismo modo que ya quedaban fuera del `ZADD` en 1.3
